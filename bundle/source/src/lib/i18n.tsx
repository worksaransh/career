...
export function I18nProvider({ children }: { children: React.ReactNode }) {
  ...
  React.useEffect(() => {
    ...
    const saved = window.localStorage.getItem("disha.lang") as Lang | null;
    if (saved) setLangState(saved);
  }, []);
  const setLang = React.useCallback((l: Lang) => {
    setLangState(l);
    if (typeof window !== "undefined") window.localStorage.setItem("disha.lang", l);
  }, []);
  const t = React.useCallback((k: Key) => STRINGS[k][lang], [lang]);
