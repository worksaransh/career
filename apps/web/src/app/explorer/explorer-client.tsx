...
import { Search, TrendingUp, DollarSign, Shield, ExternalLink, HelpCircle } from "lucide-react";
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {filtered.length === 0 && (
          <div className="col-span-full text-center py-12 text-muted-foreground">
            No matching {type.toLowerCase()}s found.
          </div>
        )}
      </div>
      ...
      {page.stats && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">Explore Stats</CardTitle>
            </CardHeader>
            <CardContent className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center">
                <TrendingUp className="h-5 w-5 mx-auto mb-1 text-primary" />
                <div className="text-xl font-bold">{page.stats.totalCareers || 2500}</div>
                <div className="text-xs text-muted-foreground">Careers</div>
              </div>
              <div className="text-center">
                <Shield className="h-5 w-5 mx-auto mb-1 text-primary" />
                <div className="text-xl font-bold">{page.stats.totalColleges || 800}</div>
                <div className="text-xs text-muted-foreground">Colleges</div>
              </div>
              <div className="text-center">
                <DollarSign className="h-5 w-5 mx-auto mb-1 text-primary" />
                <div className="text-xl font-bold">
                  ₹{((page.stats.averageSalary || 600000) / 100000).toFixed(1)}L
                </div>
              ...
              <div className="text-center">
                <TrendingUp className="h-5 w-5 mx-auto mb-1 text-primary" />
                <div className="text-xl font-bold">
                  {(page.stats.topDemandCareers || []).length || 15}
                </div>
                <div className="text-xs text-muted-foreground">Top Demand</div>
              </div>
            </CardContent>
          </Card>
        </motion.div>
      ...
      {page.cta && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="flex gap-3 justify-center">
          <Button asChild>
            <a href={page.cta.primary.url}>{page.cta.primary.text}</a>
          </Button>
          <Button variant="outline" asChild>
            <a href={page.cta.secondary.url}>{page.cta.secondary.text}</a>
          </Button>
        </motion.div>
      )}
    </div>
